...
void scan :: polygon(int n)
{
    ...
    x[n]=x[0];
    y[n]=y[0];
    for(int i=0 ; i<n ; i++)  //Draw all line (edge of polgon)
    {
        line(x[i] , y[i] , x[i+1] , y[i+1]);  //Line coordinates x1,y1,x2,y2
    }

    for(i=0 ; i<n ; i++)  //finding slopes of all lines
    {
    ...
    for (int p=0 ; p<480 ; p++) //consider 480 horizontal lines on screen
    {
        ...
        for (i=0 ; i<n ; i++)
        {
            ...
            {
                x_int[k]=x[i]+slope[i]*(p-y[i]);  //find out intersection point using formula
                k++;
            }
        }
